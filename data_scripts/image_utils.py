...
def imread(filename, target_shape=None, interpolation=cv2.INTER_AREA):
    ...
    if target_shape is not None:
        assert len(
            target_shape) == 2, 'Parameter target_shape must be 2-dimensional'
...
def adjust_saturation_and_value(im, saturation=0, value=0):
    ...
    if saturation != 0:
        if saturation > 0:
            s = np.where(s <= 255-saturation, s +
                         saturation, 255).astype('uint8')
        else:
...
def pad_or_crop(im, target_shape, cval=0):
    ...
    if w_pad > 0:
        ...
        if len(im.shape) == 3:
            padding += ((0, 0),)
        im_padded = np.pad(im_padded, padding,
...
def random_rotate(im, gt, rotation_range, zoom_range, ignore_label):
    ...
    y = rotate(gt, angle, scale=zoom,
               interpolation=cv2.INTER_NEAREST, cval=ignore_label)
    return x, y
...
def labels2colors(labels, cmap):
    ...
    for label in np.unique(labels):
        ...
        label_mask = np.dot(np.expand_dims(label_mask, axis=2),
                            np.array(cmap[label]).reshape((1, -1)))
...
def apply_mask(im, im_pred):
    ...
    alpha_channel -= 127 * \
        np.all(im_pred == np.array([0, 0, 0]), axis=2).astype(b_channel.dtype)
...
def random_transform(im, gt, target_shape, saturation_range=None, value_range=None,
    ...
    saturation = 0 if saturation_range is None else np.random.randint(
        *saturation_range)
    value = 0 if value_range is None else np.random.randint(*value_range)
    x = adjust_saturation_and_value(x, saturation, value)

    # Adjust brightness and contrast
    brightness = 0 if brightness_range is None else np.random.randint(
        *brightness_range)
    contrast = 0 if contrast_range is None else np.random.randint(
        *contrast_range)
