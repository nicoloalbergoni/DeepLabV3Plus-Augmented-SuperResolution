...
from data_scripts.image_utils import (imread, imwrite, bytesread,
...
class PascalVOC2012Dataset():
    ...
    def color_map_viz(self, class_labels):
        ...
        array = np.empty((row_size*(n_classes+1), col_size,
                         cmap.shape[1]), dtype=cmap.dtype)
        for i in range(n_classes):
        ...
        plt.imshow(array)
        plt.yticks(
            [row_size*i+row_size/2 for i in range(n_classes+1)], class_labels)
        plt.xticks([])
        plt.show()
    ...
    def parse_record(self, record_serialized):
        ...
        parsed = tf.io.parse_single_example(
            serialized=record_serialized, features=keys_to_features)

        # Decode the raw data
        im = tf.image.decode_png(parsed['image_raw'])
        gt = tf.io.decode_raw(parsed['label_raw'], tf.uint8)
        gt = tf.reshape(gt, [tf.cast(parsed['height'][0], tf.int64), tf.cast(
            parsed['width'][0], tf.int64)])
    ...
    def predict_dataset(self, save_path, dataset_filepath, model, batch_size):
        ...
        output_path = os.path.join(
            save_path, datetime.now().isoformat().split('.')[0]).split(':')
        output_path = output_path[0] + ':' + \
            output_path[1] + 'H' + output_path[2]
        ...
        while True:
            try:
                ...
                im_softmax = im_softmax.reshape(
                    (len(im_batch), np.prod(model.image_shape), self.n_classes+1))

                for i in range(len(im_batch)):
                    # Predict pixel class and expand with a channel dimension.
                    im_pred = np.argmax(im_softmax[i], axis=1).reshape(
                        model.image_shape)
                    im_pred = labels2colors(im_pred, self.cmap)
                    im_masked = center_crop(apply_mask(
                        im_batch[i], im_pred), shape_batch[i][:2])
                    imwrite(os.path.join(output_path,
                            str(idx) + '.jpg'), im_masked)
                    idx += 1
